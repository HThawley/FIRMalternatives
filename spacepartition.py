...
class Spacepartition:
    ...
    def _update_resolution(self):
        ...
        if len(self.childless) > 0:
            resolved_mask = semibarren_speedup(tuple(self.childless), np.ones(self.ndim, dtype=np.bool_), self.min_half_length) 
    ...
    def _get_noptimal(self):
        ...
        if best.sum() > 0:
            ...
            best = ~semibarren_speedup(tuple(self.childless[best]), self.dims, self.min_half_length)
    ...
    def _get_nearoptimal_neighbours(self):
        ...
        if self.noptimal_resolved.sum() > 0 and len(self.childless) > 0:
            ...
            self.eligible = ~semibarren_speedup(tuple(self.childless), self.dims, self.min_half_length)
            if self.eligible.sum() > 0:
                # rectangles failing beyond maximal extent of near-optimal resolved are ineligible
                self.eligible[self.eligible] = ~_borderheuristic(tuple(self.childless[self.eligible]), 
                                                                 tuple(self.edge_resolved[self.noptimal_resolved]), 
                                                                 self.lb==self.ub)
    ...
    def _sort_new_children(self):
        ...
        if lnh > 0:
            ...
            self.resolved_mask = semibarren_speedup(tuple(self.new_hrects), np.ones(self.ndim, dtype=np.bool_), self.min_half_length)
    ...
    def _find_neighbours_parent(self, mask):
        return find_neighbours(
            tuple(self.childless[self.eligible * mask]), 
            tuple(self.edge_resolved[self.noptimal_resolved])
            )
    ...
    def _sort_by_sum(self, mask):
        return landlocked_bysum(
            tuple(self.edge_resolved[mask]),
            tuple(self.all_resolved),
            self.bounds
            )
    ...
    def _sort_by_contra(self, mask): 
        return landlocked_bycontra(
            tuple(self.edge_resolved[mask]),
            tuple(self.childless)
            )  
        
    def _sort_nop_by_contra(self, mask): 
        return landlocked_bycontra(
            tuple(self.edge_noptimal[mask]),
            tuple(self.childless)
            )  
